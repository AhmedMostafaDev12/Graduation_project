...
class UnifiedTaskExtractor:
    ...
    def _extract_tasks_with_processor(
        ...
        """
        if processor == 'audio':
            from sentry_app.services.task_extraction.audio_processor import process_audio
            ...
            from sentry_app.services.task_extraction.document_processor import process_document
            tasks = process_document(str(file_path))
            return tasks if tasks else []

        elif processor == 'vision':
            from sentry_app.services.task_extraction.vision_extractor import extract_tasks_from_image
...
from sentry_app.services.task_extraction.unified_task_extractor import extract_tasks_from_file
from sqlalchemy.orm import Session
...
from sentry_app.services.task_extraction.unified_task_extractor import extract_tasks_to_json_file
