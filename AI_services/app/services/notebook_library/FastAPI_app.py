...
# Load environment variables BEFORE importing modules that use settings
load_dotenv()
...
app = FastAPI(
    title="Notebook Library API",
    description="An API for creating notebooks, uploading materials, and interacting with an AI assistant.",
    version="1.0.0"
)
...
@app.get("/")
async def root():
    return {"message": "Notebook Library API"}
